...
QUnit.module('ColorUtil', () => {
  ...
  QUnit.module('categorizeSchemes', () => {
    QUnit.test('sorts allEqual before someEqual before noneEqual', (assert) => {
      ...
      const result = ColorUtil.categorizeSchemes([none, some, all]);
    ...
    QUnit.test('preserves relative order within each category', (assert) => {
      ...
      const result = ColorUtil.categorizeSchemes([scheme1, scheme2]);
    ...
    QUnit.test('non-adjacent match sorts before noneEqual without requireAdjacency', (assert) => {
      ...
      const result = ColorUtil.categorizeSchemes([noMatch, nonAdjacent], false);
    ...
    QUnit.test('non-adjacent match is ignored with requireAdjacency', (assert) => {
      ...
      const result = ColorUtil.categorizeSchemes([nonAdjacent, adjacent], true);
